...
class syntax_plugin_graphviz extends DokuWiki_Syntax_Plugin {
    ...
    function handle($match, $state, $pos, Doku_Handler $handler) {
        ...
        if(preg_match('/\b(svg)\b/i',$conf,$match)) $return['render'] = 'svg';
        if(preg_match('/\b(png)\b/i',$conf,$match)) $return['render'] = 'png';
    ...
    function render($format, Doku_Renderer $R, $data) {
        ...
        if($format == 'xhtml'){
            if($render == 'svg') {
                $svg = DOKU_BASE.'lib/plugins/graphviz/svg.php?'.buildURLparams($data);
                //$R->doc .= '<object data="'.$svg.'" type="image/svg+xml" width="1000" height="1000"><img src="yourfallback.jpg" /></object>';
                $R->doc .= '<embed src="'.$svg.'" type="image/svg+xml" ';
